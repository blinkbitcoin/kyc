...
make e2e-live       # check → E2E Postgres (:5433) → live tests → the access-token
                    #   example on :4100 (TOKEN_PORT) mints a real token → DB down
```
...
| `port 4100 is taken` / the example never answers | a stale example process | `TOKEN_PORT=4110 make e2e-live` |
