...
The `ErrorCode` GraphQL enum in `packages/kyc-service/schema.graphql` is the wire contract; `make codegen` regenerates the client's copy and a parity test fails on drift.
