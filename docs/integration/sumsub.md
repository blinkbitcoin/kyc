...
| 3.5 | **Full happy path** | Submit a sandbox document plus liveness | Outcome line reads `completed: pending` (or `approved` if the level auto-approves). `verification_session.status` moves off `initial`. The state half of 3.5-3.8 (binding, approve, retry, final) is what `make e2e-live` proves without a camera |
