...
// script. Every page a provider renders inlines this string first and then
// its own provider script in the SAME nonced <script> block, so the bridge
// is the one place the wire protocol is spoken and the pages are plain
// consumers of window.__kycBridge.
//
// BRIDGE_SOURCE and BRIDGE_PROTOCOL_VERSION are the values
// @blinkbitcoin/kyc-core's bridge module publishes; the page test pins them
// to the package so the two cannot drift.
//
// The script is a string, so bridge/__tests__/script.test.ts runs it the way
// the browser would - new Function('window', BRIDGE_SCRIPT)(fakeWindow) - and
