...
const install = (window: Partial<FakeWindow> = {}): InstalledBridge => {
  ...
  // eslint-disable-next-line no-new-func -- the bridge ships as a string; running it is the test
...
const iframe = (): { window: Partial<FakeWindow>; postMessage: jest.Mock } => {
  const postMessage = jest.fn();
...
describe('the bridge script installs window.__kycBridge', () => {
  ...
  it('merges into a bridge whose setToken is an accessor, without disturbing it', () => {
    ...
    const { window, bridge } = install({
      __kycBridge: existing as Partial<BridgeApi>,
    });
...
describe('post', () => {
  ...
  it('carries the payload when there is one, and omits the key when there is not', () => {
    ...
    const sent = host.postMessage.mock.calls.map(([body]) =>
  ...
  it('falls through to the parent when ReactNativeWebView cannot post', () => {
    ...
    const host = iframe();
    install({ ...host.window, ReactNativeWebView: {} }).bridge.post(
      'submitted',
    );
...
describe('readToken', () => {
  ...
  it('takes the kyc-core setToken envelope', () => {
    ...
    expect(
      install().bridge.readToken({ ...envelope('setToken'), token: 'token-2' }),
    ).toBe('token-2');
  });
...
describe('the inbound message listener', () => {
  ...
  it.each([
    ...
    [
      'another library on the same page',
      { source: 'other-bridge', v: 1, type: 'setToken' },
    ],
    [
      'a future protocol version',
      { source: BRIDGE_SOURCE, v: 2, type: 'setToken' },
    ],
  ...
  it('does nothing when no page handler is installed yet', () => {
    ...
    const installed = install();
    expect(() =>
      installed.dispatch({ ...envelope('setToken'), token: 'token-1' }),
    ).not.toThrow();
