...
| `codeql.yml` | Push to main, PRs (both skip docs-only changes via the<br>same `Changes` classifier), weekly schedule | CodeQL static analysis (JavaScript/TypeScript); alerts land under Security → Code scanning;<br>suite + alert-suppression query in `.github/codeql/codeql-config.yml`;<br>`make codeql` runs the same analysis locally |
