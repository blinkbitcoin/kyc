...
This is the only mode that needs `@apollo/client` and `graphql`, and the only one that needs **this repo's** `apps/api`.
...
The service refuses to boot if any of the required secrets is missing (see [../architecture/security.md](../architecture/security.md)). For local work against the mock provider, `KYC_PROVIDER=mock` plus `ALLOW_INSECURE_DEV=true` is the intended shortcut and is what `apps/api/.env.test` uses.
...
The bearer token is your app's, not the provider's. With `JWT_SECRET` set, `apps/api` verifies it as HS256 and uses the `sub` claim as the user id; every session read and refresh is owner-scoped against it. In development without `JWT_SECRET`, the raw token is the user id - which is why the demos simply send `demo-user`.
