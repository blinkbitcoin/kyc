...
free on a public repo; on a private repo macOS bills at 10x Linux (one ~7 min
run is ~70 Linux minutes), which is why the job was opt-in before the repo
...
What keeps the job at ~7 min on a 3-vCPU hosted runner (it was ~15): the
simulator boots first and only downloads overlap it, Metro starts after the
boot, Metro's transform cache is restored from the last green run
(`METRO_CACHE_ROOT`, `scripts/e2e/metro-start.sh`), and the prewarm requests
the exact bundle options the dev client asks for, so the app-launch flow is
served from the warm graph instead of building its own. Change the order or
the prewarm URL only with the step timings of a run in hand.
