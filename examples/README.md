...
|---------|-------|-------|-----|
| [`full-service-demo/`](full-service-demo/README.md) | 🖥️ `@blinkbitcoin/kyc-node` (Express 5 + Apollo 5 + Knex/Postgres) | the whole service: `mock` \| `sumsub` via `KYC_PROVIDER` | Vitest against real Postgres (`make e2e-backend`); the backend every other suite runs against |
