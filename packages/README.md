...
| [`kyc-service/`](kyc-service/README.md) | `.`, `./node`, `./vercel`,<br>`./cloudflare` | 🖥️ **The deployable.** The whole service on `kyc-node` as one Fetch<br>core: access tokens always, sessions (Apollo + Postgres, the hosted<br>page, the webhook) with `DATABASE_URL`, this service's session<br>verification, CORS, rate limits and fail-closed boot around the<br>package. Needed for mode 3 only; the backend every E2E suite runs<br>against |
